    ...
    <div class="contenu flex flex-row relative px-56 pt-8 gap-100">
        <div class="py-20 w-[17.75rem]">
            ...
            {:else}
                ...
                <img src={URL.createObjectURL(file)} alt="avatar" class="rounded-full w-[17.75rem] h-[17.75rem]" />
                {/each}
            {/if}
            <div class="inputfile">
        ...
        <div class="bg-[#FBFBFB] w-[48.75rem] h-[44rem] rounded-lg p-8">
            ...
            <div class="flex items-center gap-4">
                ...
                <button class="underline text-[1rem] text-[#B7B7B7]" on:click={() => modify = true}>Changer de pseudo</button>
            </div>
            <div class="pt-4 flex gap-4 items-center">
            {#if modify}
            <TextBox placeholder="Pseudo" bind:value={user.pseudo} showIcon={false}/>
            <button class="text-[1rem] text-[#B7B7B7] underline valider" on:click={() => modify = false}><svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
