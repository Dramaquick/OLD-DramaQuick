<script>
    ...
    if (user.avatar === null) {
        lastAvatar = "https://cdn.discordapp.com/embed/avatars/0.png";
    }
    else {
    ...
    function updateAvatar() {
        ...
        if (files.length === 0) {
            ...
            modify = false;
            notify("Erreur", "L'image ne peut pas dépasser 1Mo", "error", 5000, "box", "corner-top-right");
            return
        }
        else if (files[0].type !== "image/png" && files[0].type !== "image/jpeg") {
            modify = false;
            notify("Erreur", "L'image doit être au format png ou jpeg", "error", 5000, "box", "corner-top-right");
            return
        }
        else {
            ...
            modify = false;
            notify("Modification", "L'avatar a bien été modifié", "success", 5000, "box", "corner-top-right")
        }
    }
    ...
    function infosToUser () {
        if (modify === "pseudo") {
            if (infos.pseudo === user.pseudo) {
                modify = false;
                return
            }
            else if (infos.pseudo === "") {
                infos.pseudo = user.pseudo;
                modify = false;
                return
            }
            else if (infos.pseudo.length > 20) {
                infos.pseudo = user.pseudo;
                modify = false;
                notify("Erreur", "Le pseudo ne peut pas dépasser 20 caractères", "error", 5000, "box", "corner-top-right");
                return
            }
            user.pseudo = infos.pseudo;
            modify = false;
            notify("Modification", "Le pseudo a bien été modifié", "success", 5000, "box", "corner-top-right")
        }
        else if (modify === "email") {
            ...
            else if (infos.email.length > 50) {
                infos.email = user.email;
                modify = false;
                notify("Erreur", "L'email ne peut pas dépasser 50 caractères", "error", 5000, "box", "corner-top-right");
                return
            }
            user.email = infos.email;
            modify = false;
            notify("Modification", "L'email a bien été modifié", "success", 5000, "box", "corner-top-right")
        }
    }
...
<main class="h-screen w-full overflow-hidden bg-no-repeat">
    ...
    <div class="contenu flex flex-row relative px-56 pt-8 gap-100">
        <div class="py-20 w-[17.75rem]">
            <img src={user.avatar} alt="avatar" class="rounded-full w-[17.75rem] h-[17.75rem]" />
            <div class="inputfile">
                <input type="file" id="file" accept="image/png, image/jpeg"
                bind:files
                name="avatar"
                class="hidden"
                on:click = {() => {lastAvatar = user.avatar;modify = "avatar";}}
        ...
        <div class="bg-[#FBFBFB] w-[48.75rem] h-[44rem] rounded-lg p-8">
            ...
            <button class="text-[1rem] text-[#B7B7B7] underline valider" on:click={() =>  infosToUser()}><svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
